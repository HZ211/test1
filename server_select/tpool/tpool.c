...
static void* thread_routine(void *arg)
{
    ...
    while(1) {
        ...
        while(!tpool->queue_head && !tpool->shutdown) {   //head  == NULL && shutdown == 0 : 线程池里的线程就阻塞等待
            pthread_cond_wait(&tpool->queue_ready, &tpool->queue_lock);
        }

		/*查看线程池开关，如果线程池关闭，线程退出*/
        if (tpool->shutdown) {  //在销毁线程池时才会工作
            pthread_mutex_unlock(&tpool->queue_lock);
            pthread_exit(NULL);
        }

		/*从任务链表中取出任务，执行任务*/
        work = tpool->queue_head;   //从任务链表头部取出任务，任务链表指向下一个节点
        tpool->queue_head = tpool->queue_head->next;
        pthread_mutex_unlock(&tpool->queue_lock);  //互斥锁打开
        work->routine(work->arg);  //调用任务函数执行任务
...
int tpool_add_work(void*(*routine)(void*), void *arg)
{
    ...
    work->routine = routine;  //把任务函数添加到任务链表里边
    work->arg = arg;            //参数
    work->next = NULL;
...
//		printf("not first work in work-queue\n");
    ...
    /* 通知工作者线程，有新任务添加 */
    pthread_cond_signal(&tpool->queue_ready);  //这一步是将阻塞等待的任务链表唤醒，执行新到来的任务
    pthread_mutex_unlock(&tpool->queue_lock);
    return 0;
}
